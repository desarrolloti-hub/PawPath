...
import { sendChatMessage } from '/config/groq-service.js';
import { obtenerResumenContextoActivo } from '/config/chatbot-data-service.js';
...
(function () {
    ...
    const MAX_HISTORIAL = 12; // 6 intercambios completos en crudo; el resto se condensa
    ...
    class PawBotComponent extends HTMLElement {
        ...
        /**
         * Detecta si la página actual tiene una mascota seleccionada (ej. detalle de
         * mascota vía ?id=) y, si pertenece/corresponde al usuario autenticado, la
         * asume automáticamente como contexto para evitar preguntas innecesarias.
         */
        async _cargarContextoActivo() {
            const { rol, uid } = this._contextoRol;
            if (!uid || rol === 'invitado') return;

            const mascotaId = new URLSearchParams(window.location.search).get('id') || undefined;
            if (!mascotaId) return;

            try {
                this._contextoRol.contextoActivoTexto = await obtenerResumenContextoActivo({ rol, uid, mascotaId });
            } catch {
                // Si falla la consulta de contexto, se continúa sin él (no bloqueante).
            }
        }
        ...
        async _enviar(texto) {
            ...
            try {
                this._contextoRol.resumenPrevio = this._resumenPrevio || undefined;
        ...
        /**
         * Mantiene acotado el historial en memoria: cuando crece demasiado, descarta
         * los intercambios más antiguos y conserva solo un resumen breve de sus temas
         * (sin llamar al modelo), para no reenviar contexto irrelevante ni gastar tokens.
         */
        _podarHistorial() {
            if (this._historial.length <= MAX_HISTORIAL) return;

            const exceso = this._historial.length - MAX_HISTORIAL;
            const descartados = this._historial.splice(0, exceso);
            const temas = descartados
                .filter(m => m.role === 'user')
                .map(m => (m.parts?.[0]?.text || '').slice(0, 60).trim())
                .filter(Boolean);

            if (temas.length) {
                this._resumenPrevio = this._resumenPrevio
                    ? `${this._resumenPrevio} ${temas.join('; ')}.`
                    : `El usuario ya preguntó antes sobre: ${temas.join('; ')}.`;
            }
        }
        ...
        _bindEvents() {
            ...
            clear?.addEventListener('click', () => {
                this._historial = [];
                this._resumenPrevio = '';
                if (this._contextoRol) this._contextoRol.resumenPrevio = undefined;
            ...
            window.addEventListener('storage', (e) => {
                if (e.key === 'currentUserRole' || e.key === 'userDisplayName' || e.key === 'currentUserId') {
                    this._cargarContextoUsuario();
                    this._cargarContextoActivo();
                }
            });
